...
public class CassandraRepository {
	...
	static final Logger logger = LoggerFactory
			.getLogger(CassandraRepository.class);
	 ...
	 * @return
	 */
	public static CassandraRepository getInstance(String host, String keyspace)
			throws Exception {
		if (instance == null) {
		...
		if (metadata != null) {
			...
			if (keyspaceMetadata.getTable("rules") == null) {
				throw new UnsupportedOperationException(
						"Can't find table logging in " + instance.keyspace);
			}
			if (keyspaceMetadata.getTable("events") == null) {
				throw new UnsupportedOperationException(
						"Can't find table endpoint in " + instance.keyspace);
			}
			if (keyspaceMetadata.getTable("condition_exception") == null) {
				throw new UnsupportedOperationException(
						"Can't find table condition_exception in "
								+ instance.keyspace);
			}
			if (keyspaceMetadata.getTable("result_computation") == null) {
				throw new UnsupportedOperationException(
						"Can't find table result_computation in "
								+ instance.keyspace);
			}
	...
	public List<Event> getEvent(long startTime) throws Exception {
		String sql = "SELECT * FROM " + keyspace
				+ ".events where time > ? ALLOW FILTERING;";
		List<Object> params = new ArrayList<Object>();
	...
	public List<Event> getEvent(long startTime, String eventName)
			throws Exception {
		String sql = "SELECT * FROM " + keyspace
				+ ".events where event_name = ? and time > ? ALLOW FILTERING;";
		List<Object> params = new ArrayList<Object>();
	...
	public List<Event> getEvent(String hashkey) throws Exception {
		String sql = "SELECT * FROM " + keyspace
				+ ".events where hashkey = ? ALLOW FILTERING;";
		List<Object> params = new ArrayList<Object>();
		params.add(hashkey);
		ResultSet rows = execute(sql, params);
		List<Event> results = null;
	 ...
	 * 
	 * @param id
	 *            : id=-1 --> skip this field
	 * @param eventName
	 * @param ruleName
	 * @return
	 * @throws Exception
	 */
	public List<EventQuery> getEventQueryFromDB(String eventName,
		...
		for (Row row : rows) {
			...
			eventQuery.setStatus(EventQueryStatus.valueOf(row
					.getString("status")));
	...
	public void changeEventQueryStatus(EventQuery eventQuery) throws Exception {
		String sql = "UPDATE "
				+ keyspace
				+ ".rules set status = ? where event_name = ? AND rule_name = ?";
		List<Object> params = new ArrayList<Object>();
	...
	public void deleteEventQuery(EventQuery eventQuery) throws Exception {
		String sql = "DELETE FROM " + keyspace
				+ ".rules WHERE event_name = ? and rule_name = ?";
		List<Object> params = new ArrayList<Object>();
		params.add(eventQuery.getEventName());
		params.add(eventQuery.getRuleName());
		execute(sql, params);
	 ...
	 * @throws Exception
	 */
	public List<EventQueryException> getEventQueyExceptionNotExpired(
			EventQuery eventQuery, String actionStr) throws Exception {
		...
		for (Row row : rows) {
			...
			ExptionAction action = EventQueryException.ExptionAction
					.getContentType(row.getString("action"));
			Date expiredDate = row.getDate("expired_date");
			Map<String, Object> conditionFilter = JsonHelper
					.toMap(new JSONObject(row.getString("condition_filter")));
			eventQueryException = new EventQueryException(eventName, ruleName,
					action, expiredDate, conditionFilter);
			results.add(eventQueryException);
		}
		return results;
	}

	public void insertEventQueryException(
			EventQueryException eventQueryException) throws Exception {
		String sql = "INSERT INTO "
				+ keyspace
	 ...
	 * @throws Exception
	 */
	public void insertResultComputation(String eventName, String rule,
			long time, String hashKey, String result) throws Exception {
		String sql = "INSERT INTO "
				+ keyspace
	 ...
	 * @throws Exception
	 */
	public void insertResults(String eventName, String hashKey,
			List<String> result, List<String> filterResult) throws Exception {
		String sql = "INSERT INTO "
				+ keyspace
	...
	public void updateResults(EventResult eventResult) throws Exception {
		...
		if (eventResult.getFilteredResult() != null
				&& eventResult.getFilteredResult().size() > 0) {
			params.add(eventResult.getFilteredResult());
		} else {
			params.add(result);
		}
	 ...
	 * @throws Exception
	 */
	public List<EventResult> getEventResult(String eventName, String hashkey)
			throws Exception {
		String sql = "SELECT * FROM " + keyspace
				+ ".results WHERE event_name = ? and hashkey = ?;";
		List<Object> params = new ArrayList<Object>();
		params.add(eventName);
		params.add(hashkey);
		ResultSet rows = execute(sql, params);
		...
		for (Row row : rows) {
			...
			eventResult = new EventResult(row.getString("event_name"),
					row.getString("hashkey"), row.getList("result",
							String.class).toString(), row.getList(
							"filtered_result", String.class).toString());
	...
	public List<EventResult> getEventResult(String hashkey) throws Exception {
		String sql = "SELECT * FROM " + keyspace
				+ ".results WHERE hashkey = ? ALLOW FILTERING;";
		List<Object> params = new ArrayList<Object>();
		params.add(hashkey);
		ResultSet rows = execute(sql, params);
		List<EventResult> results = null;
	...
	public ResultSet execute(String sql, List<Object> listParams)
		...
		try {
			if (listParams != null && listParams.size() > 0) {
				...
				res = session
						.execute(boundStatement.bind(listParams.toArray()));
			} else {
	...
	public List<EventQuery> getEventQueryFromDB(List<String> lstEventName)
			throws Exception {
		StringBuilder sql = new StringBuilder("SELECT * FROM " + keyspace
				+ ".rules");
		List<Object> params = new ArrayList<Object>();
