...
public class EventProcessorServiceAdminTest {
  ...
  public static void beforeClass() {
    ...
    try {
      service =
          new EventProcessorServiceAdmin(CassandraRepository.getInstance(
              ccServiceFactory.getDbHost(), ccServiceFactory.getDbName()), ccServiceFactory);
    } catch (Exception e) {
  ...
  public void testAddRuleException() {
    try {
      service.addRuleException("test_event", "test_rule_name",
          EventQueryException.ExptionAction.VERIFIED.toString(), "12/12/2018 12:00:00", "");
      EventQuery eventquery = new EventQuery();
      eventquery.setEventName("test_event");
      eventquery.setRuleName("test_rule_name");
      List<EventQueryException> list =
          CassandraRepository.getInstance(ccServiceFactory.getDbHost(),
              ccServiceFactory.getDbName()).getEventQueyExceptionNotExpired(eventquery,
              EventQueryException.ExptionAction.VERIFIED.toString());
  ...
  public void testAddUpdateAndDeleteRule() {
    try {
      service.addRule("test_event", "test_rule_name", "test_event", "amount:int", "amount > 0", "",
          "", 0, 0, "1 minute", "", "amount > 10", "", EventQueryStatus.STOP);
      ...
      service.updateRule("test_event", "test_rule_name", "test_event", "amount:double",
          "amount > 0", "", "", 0, 0, "1 minute", "", "amount > 10", "", false);
      list =
          CassandraRepository.getInstance(ccServiceFactory.getDbHost(),
              ccServiceFactory.getDbName()).getEventQueryFromDB("test_event", "test_rule_name");
      if (list != null && !list.isEmpty()) {
